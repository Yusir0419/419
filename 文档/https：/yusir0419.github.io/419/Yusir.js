...
const apiKey = "9GiVt4KkoxtdQBDS"  //彩云天气apiKey，自行申请
...
const rainycolor = new Color(gColor, 1) //下雨天温度字体颜色
...
let theWeek = "第"+ Math.ceil((parseInt((date - beginDate) / (24 * 60 * 60 * 1000)) + 1 + beginDate.getDay()) / 7) + "周"
log("开始时间"+beginDate)
...
drawText(topDrawing,datePointX, 28, 250, 24, currentDate.getFullYear().toString().concat("年").concat(dateFormat)+" "+theWeek, timecolor, "regular",20,"left") 
...
// 温度+"º"
drawText(weatherDrawing, firstPointtoLeft+deltaX*i*2.15-20-355-5,150-98+10, 40,20,Math.round(data[i*2].value).toString(),temperaturetextcolor,"regular",16,"center")
...
drawText(weatherDrawing, firstPointtoLeft+deltaX*i*2.10-20-355-5, 100-98-4, 40, 30, weathertimeText,temperaturetextcolor,"regular",16,"center")
...
// log(weatherDate)
df.dateFormat = "E"
drawText(weatherDrawing, 5+88*(i-1)-1,173-98+24, 50,20,df.string(weatherDate),daycolor,"a",16,"center")
}
...
drawing.setFont(Font.regularSystemFont(fontsize))}
  ...
  if(alignment == "right")
  {drawing.setTextAlignedRight()}drawing.drawTextInRect(text, new Rect(x, y, width, height))
...
async function getLocationArea() {  			
	// 加个缓存
	let cachePath = files.joinPath(files.documentsDirectory(), "areaCache-Pih");		
